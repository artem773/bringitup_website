...
window.addEventListener('DOMContentLoaded', function () {
  ...
  slider.render();
  var moduleNextPageSlider = new _modules_slider_slider_main__WEBPACK_IMPORTED_MODULE_0__["default"]({
    container: '.moduleapp',
    btns: '.next'
  });
...
function () {
  ...
  _createClass(Form, [{
    ...
    value: function initMask() {
      var setCoursorPosition = function setCoursorPosition(position, element) {
        element.focus();
...
function () {
  ...
  _createClass(VideoPlayer, [{
    ...
    value: function onPlayerStateChange(state) {
      //когда заканчівается відео, ета фун-я разблокіривает второе відео на страніце 
      try {
        var blockedElement = this.activeBtn.closest('.module__video-item').nextElementSibling; // получаєм следующій елемент по верстке

        var playButton = this.activeBtn.querySelector('svg').cloneNode(true); //глубокое копірованіе всех елементов і кнопкі ПЛЕЙ 
    ...
    value: function init() {
      if (this.buttons.length > 0) {
        ...
        firstScriptTag.parentNode.insertBefore(tag, firstScriptTag); //подключілі АРІ із с 'https://developers.google.com/youtube/iframe_api_reference?hl=ru'
